// VENDORED FROM fheroes2 -- see UPSTREAM_REV in this directory for the revision this was taken
// from. This file replaces src/fheroes2/battle/battle_only.cpp in the build; the upstream copy is
// excluded by battle/CMakeLists.txt. The fheroes2 submodule itself is never modified.
//
// Every deviation from upstream is marked with a "LOCAL CHANGE" comment. To re-sync after moving
// to a newer fheroes2 revision, diff this file against the upstream one and re-apply those hunks:
//
//     diff -u ../../fheroes2/src/fheroes2/battle/battle_only.cpp battle_only.cpp
//
// Current local changes: each side can be played by a human, by the game AI, or by an external
// harness, instead of the attacker always being human and the defender being a human/AI toggle.
...
// LOCAL CHANGE: needed to read and cycle the per-side control modes.
...
namespace
{
    ...
    // LOCAL CHANGE: the control type of a side follows its configured mode. A side played by the
    // harness is marked as AI-controlled, because as far as the engine's turn loop is concerned it
    // is not a human at the keyboard; the external AI hook then intercepts those turns.
    int controlTypeForSide( const int side )
    {
        return ( ExternalAI::sideMode( side ) == ExternalAI::ControlMode::HUMAN ) ? CONTROL_HUMAN : CONTROL_AI;
    }
    ...
    int32_t getMaxDefendingHeroAreaWidth()
    {
        ...
        // LOCAL CHANGE: the widget now shows a mode name rather than always saying "Human", so it
        // has to be wide enough for the longest of them.
        int32_t widest = 0;
        for ( const char * name : { "Human", "Game AI", "External AI" } ) {
            fheroes2::Text text( name, fheroes2::FontType::smallWhite() );
            text.fitToOneRow( defendingHeroTypeNameMaxWidth );

            widest = std::max( widest, text.width() );
        }

        return cell.width() + 5 + widest;
    }

    // LOCAL CHANGE: renders the control mode of a side. The checkbox is ticked only for a human
    // player; the label spells out which of the three modes is active.
    void renderHeroTypeUI( const int side, const fheroes2::Point offset, const bool isEvilInterface, fheroes2::Image & output )
    {
        const fheroes2::Sprite & cell = Assets::getImage( ICN::CELLWIN, 1 );

        fheroes2::Blit( cell, output, offset.x, offset.y );
        if ( ExternalAI::sideMode( side ) == ExternalAI::ControlMode::HUMAN ) {
        ...
        fheroes2::Text text( ExternalAI::sideModeName( side ), fheroes2::FontType::smallWhite() );
        text.fitToOneRow( defendingHeroTypeNameMaxWidth );
...
Battle::Only::Only()
{
    ...
    // LOCAL CHANGE: the control type of each side now comes from the configured mode rather than
    // being fixed. A side played by the harness is marked as AI-controlled so that the engine's
    // turn loop routes it to the AI planner, where the external AI hook picks it up.
    for ( const int side : { ExternalAI::attackerSide, ExternalAI::defenderSide } ) {
        armyInfo[side].controlType = controlTypeForSide( side );
        armyInfo[side].player.SetControl( armyInfo[side].controlType );
        armyInfo[side].player.SetColor( playerColor[side] );
    }
}
...
bool Battle::Only::setup( const bool allowBackup, bool & resetBattleSetup )
{
    ...
    // LOCAL CHANGE: re-read the modes on every entry, so that a mode changed on the command line or
    // cycled in this screen during an earlier round is in effect for this one.
    for ( const int side : { ExternalAI::attackerSide, ExternalAI::defenderSide } ) {
        armyInfo[side].controlType = controlTypeForSide( side );
        armyInfo[side].player.SetControl( armyInfo[side].controlType );
    }
    ...
    if ( armyInfo[1].hero != nullptr ) {
        renderHeroTypeUI( ExternalAI::defenderSide, defendingHeroTypeRoi.getPosition(), isEvilInterface, display );
    }
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( armyInfo[1].hero != nullptr ) {
            // LOCAL CHANGE: clicking cycles the defender through the available modes instead of
            // toggling between human and AI. The external mode is only offered when an endpoint was
            // given on the command line, so the cycle never lands on a mode that cannot be played.
            if ( le.MouseClickLeft( defendingHeroTypeRoi ) ) {
                ExternalAI::cycleSideMode( ExternalAI::defenderSide );

                armyInfo[1].controlType = controlTypeForSide( ExternalAI::defenderSide );
                armyInfo[1].player.SetControl( armyInfo[1].controlType );

                needRedrawControlInfo = true;
            }
            else if ( le.isMouseRightButtonPressedInArea( defendingHeroTypeRoi ) ) {
                std::string msg = _( "Click to choose who plays the defending hero: a human at the keyboard, the AI that ships with the game, or an external AI." );

                if ( !ExternalAI::hasEndpoint( ExternalAI::defenderSide ) ) {
                    msg += "\n\n";
                    msg += _( "The external AI is not offered because no endpoint was given for this side. Start the game with --red external:tcp://<host>:<port> to enable it." );
                }

                fheroes2::showStandardTextMessage( _( "Defending hero" ), msg, 0 );
            }
        }
...
void Battle::Only::StartBattle()
{
    ...
    for ( const int32_t idx : { 0, 1 } ) {
        ...
        // LOCAL CHANGE: tell the external AI which colour belongs to which side, so that it can
        // recognise the turns it is responsible for once the battle starts.
        ExternalAI::bindColorToSide( armyInfo[idx].player.GetColor(), idx );
