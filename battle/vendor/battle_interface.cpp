...
// from, which is the revision setup.sh checks out. This file replaces
// src/fheroes2/battle/battle_interface.cpp in the build; the upstream copy is excluded by
// battle/CMakeLists.txt. The fheroes2 checkout itself is never modified.
//
...
//
//     diff -u ../../fheroes2/src/fheroes2/battle/battle_interface.cpp battle_interface.cpp
//
// Current local changes: the automatic combat modes (Auto Combat / Quick Combat) are disabled, so
// that no side of a battle can be handed over to the AI; a human player's commands are recorded to
// the battle log.
...
// LOCAL CHANGE: needed to record a human player's turn, see HumanTurn().
...
void Battle::Interface::HumanTurn( const Unit & unit, Actions & actions )
{
    ...
    // LOCAL CHANGE: record what the player decided, so that a human plays into the same log as the
    // game AI and the harness do.
    BattleLog::recordTurn( arena, unit, actions, "human" );
}
...
void Battle::Interface::HumanBattleTurn( const Unit & unit, Actions & actions, std::string & msg )
{
    ...
    if ( le.isAnyKeyPressed() ) {
        ...
        // LOCAL CHANGE: the automatic combat modes are disabled in this build. The hotkeys are still
        // consumed here, so that they neither ask a question whose answer would be ignored nor fall
        // through to whatever else might be bound to the same key.
        else if ( Game::HotKeyPressEvent( Game::HotKeyEvent::BATTLE_TOGGLE_AUTO_COMBAT )
                  || Game::HotKeyPressEvent( Game::HotKeyEvent::BATTLE_QUICK_COMBAT ) ) {
    ...
    if ( doListlogProcessing ) {
        ...
        // LOCAL CHANGE: the automatic combat modes are disabled in this build, so the button only
        // explains itself instead of opening a dialog whose options would do nothing.
        msg = _( "Automatic combat modes are disabled" );

        if ( le.MouseClickLeft( _buttonAuto.area() ) || le.isMouseRightButtonPressed() ) {
            fheroes2::showStandardTextMessage( _( "Automatic Combat Modes" ),
                                               _( "Automatic combat modes are disabled in this build, so that every side of the battle stays under manual control." ),
                                               Dialog::ZERO );
        }
    }
...
// LOCAL CHANGE: handing a side to the AI defeats the purpose of this build, so the two functions
// that start an automatic combat mode are disabled. Guarding them here rather than at the call
// sites covers every route into them at once: the Auto Combat button, the hotkeys, and the buttons
// of the "Automatic Combat Modes" dialog.
void Battle::Interface::_startAutoCombat( const Unit & /* unit */, Actions & /* actions */ )
{
    // Deliberately does nothing, see the note above.
}
