...
// from. This file replaces src/fheroes2/ai/ai_battle.cpp in the build; the upstream copy is
...
//
//     diff -u ../../fheroes2/src/fheroes2/ai/ai_battle.cpp ai_battle.cpp
//
// Current local changes: BattleTurn() hands a side over to the external AI when one is configured
// for its colour, and records the commands each side produced to the battle log.
...
// LOCAL CHANGE: needed by the external AI hook in BattleTurn().
#include "external_ai.h"

// LOCAL CHANGE: needed to record what each side decided, see BattleTurn().
...
void AI::BattlePlanner::BattleTurn( Battle::Arena & arena, const Battle::Unit & currentUnit, Battle::Actions & actions )
{
    ...
    //
    // LOCAL CHANGE: this check now runs before the external AI is consulted. It is the engine's
    // guard against a battle that is going nowhere, and it has to apply whoever is playing: a
    // harness that keeps skipping would otherwise keep the battle running forever.
    if ( isLimitOfTurnsExceeded( arena, actions ) ) {
        return;
    }

    // LOCAL CHANGE: a side played by the harness is marked as AI-controlled, so that Arena::TurnUnit
    // routes it here; this is where it is handed over to the harness instead of to the planner
    // below. Hooking the dispatch here rather than in Arena keeps the engine's turn loop untouched.
    //
    // Note that failures deliberately do not fall through to the planner: a harness that silently
    // turned into the game AI would look exactly like a harness that is playing well.
    if ( ExternalAI::isExternalColor( currentUnit.GetCurrentColor() ) ) {
        ExternalAI::takeTurn( arena, currentUnit, actions );

        // LOCAL CHANGE: record what the harness decided. Recording here rather than in the harness
        // means both sides are logged from the same place, in the same form, after the decision has
        // been turned into engine commands - so the log shows what the engine was actually told.
        BattleLog::recordTurn( arena, currentUnit, actions, "external" );
    ...
    // LOCAL CHANGE: see above - the game's own AI is recorded the same way.
    BattleLog::recordTurn( arena, currentUnit, actions, "game-ai" );
}
