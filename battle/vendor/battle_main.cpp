...
// from. This file replaces src/fheroes2/battle/battle_main.cpp in the build; the upstream copy is
...
//
//     diff -u ../../fheroes2/src/fheroes2/battle/battle_main.cpp battle_main.cpp
//
// Current local changes: a battle can be shown even when no human is playing it, so that a battle
// between the game's AI and the harness can be watched; the battle is bracketed in the
// engine-level battle log.
...
// LOCAL CHANGE: needed by the showBattle decision below.
#include "battle_view.h"
#include "external_ai.h" // IWYU pragma: associated
#include "battle_arena.h"

// LOCAL CHANGE: needed to bracket the battle in the engine-level log.
...
Battle::Result Battle::Loader( Army & attackingArmy, Army & defendingArmy, const int32_t tileIndex )
{
    ...
    // LOCAL CHANGE: also show the battle when asked to explicitly. A side played by the harness is
    // marked as AI-controlled, so a model-versus-AI battle looks like a battle with no human in it
    // and would otherwise be resolved without ever drawing anything.
    bool showBattle = ( !conf.BattleAutoResolve() && isHumanBattle ) || BattleView::forceShow();
    ...
    while ( true ) {
        ...
        // LOCAL CHANGE: bracket the battle in the engine-level log. The turns themselves are
        // recorded from where they are decided; this is the only place that sees the battle as a
        // whole, and it is inside the retry loop so a restarted battle starts a fresh record.
        BattleLog::recordStart( arena );

        // LOCAL CHANGE: hold the battle here, with the battlefield drawn and nothing having moved
        // yet, until the user says go. This is the only moment that works as a pause screen: before
        // the Arena exists there is nothing to look at, and after the first turn the battle has
        // already started.
        if ( showBattle && BattleView::pauseBeforeStart() ) {
            // Name both sides and who is playing them, so a recording of this shows what the
            // viewer is about to watch without any commentary.
            const auto describe = []( const Army & army ) {
                const HeroBase * commander = army.GetCommander();
                const std::string name = ( commander == nullptr ) ? std::string( _( "no hero" ) ) : commander->GetName();

                return name + " (" + ExternalAI::controllerName( army.GetColor() ) + ")";
            };

            std::string message = describe( attackingArmy ) + "\n" + _( "against" ) + "\n" + describe( defendingArmy );
            message += "\n\n";
            message += _( "Click OK to begin." );

            fheroes2::showStandardTextMessage( _( "Battle" ), message, Dialog::OK );
        }
        ...
        // LOCAL CHANGE: see recordStart() above.
        BattleLog::recordEnd( arena, result.isAttackerWin() ? "attacker" : ( result.isDefenderWin() ? "defender" : "none" ) );
